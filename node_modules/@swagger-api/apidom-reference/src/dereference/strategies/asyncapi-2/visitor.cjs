...
class AsyncAPI2DereferenceVisitor {
  ...
  async ReferenceElement(referencingElement, key, parent, path, ancestors, link) {
    ...
    if (ancestorsLineage.includes(referencedElement)) {
      ...
      if (this.options.dereference.circular === 'replace') {
        ...
        const replacer = (_this$options$derefer5 = (_this$options$derefer6 = this.options.dereference.strategyOpts['asyncapi-2']) == null ? void 0 : _this$options$derefer6.circularReplacer) != null ? _this$options$derefer5 : this.options.dereference.circularReplacer;
    ...
    if ((isExternalReference || isNonRootDocument || (0, _apidomNsAsyncapi.isReferenceElement)(referencedElement) || shouldDetectCircular || (_this$options$derefer7 = this.options.dereference.dereferenceOpts) != null && _this$options$derefer7.continueOnError) && !ancestorsLineage.includesCycle(referencedElement)) {
  ...
  async ChannelItemElement(referencingElement, key, parent, path, ancestors, link) {
    ...
    if (referencingElement === referencedElement) {
      const error = new _apidomError.ApiDOMError('Recursive Channel Item Object reference detected');
      this.indirections.pop();
      this.handleDereferenceError(error, referencingElement);
    }
    ...
    if (ancestorsLineage.includes(referencedElement)) {
      ...
      if (this.options.dereference.circular === 'error') {
        ...
        const replacer = (_this$options$derefer8 = (_this$options$derefer9 = this.options.dereference.strategyOpts['asyncapi-2']) == null ? void 0 : _this$options$derefer9.circularReplacer) != null ? _this$options$derefer8 : this.options.dereference.circularReplacer;
        const replacement = replacer(refElement);
        link.replaceWith(replacement, mutationReplacer);
